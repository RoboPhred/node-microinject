...
describe("Parameters", function () {
    describe("container.get()", function () {
        ...
        it("when an optional parameter is not supplied it injects undefined", function () {
            ...
            chai_1.expect(instance.param).to.be.undefined;
        });
    });
    describe("container.create()", function () {
        ...
        it("when an optional parameter is not supplied it injects undefined", function () {
            ...
            chai_1.expect(instance.param1).to.be.undefined;
        });
    });
    describe("factory.get()", function () {
        let container;
        let FactoryIdentifier = "factory";
        let ReceiverIdentifier = "receiver";
        ...
        let ParamReceiver = class ParamReceiver {
            constructor(paramFromContainer, paramFromFactory) {
                this.paramFromContainer = paramFromContainer;
                this.paramFromFactory = paramFromFactory;
            }
        };
        ParamReceiver = __decorate([
            injection_1.injectable(),
            __param(0, injection_1.injectParam(paramFromContainer)),
            __param(1, injection_1.injectParam(paramFromFactory))
        ], ParamReceiver);
        beforeEach(() => {
            container = new container_1.Container();
        ...
        it("Receives the container get parameter", function () {
            ...
            chai_1.expect(receiver.paramFromContainer).to.equal(paramFromContainerValue);
        });
        it("Receives the factory get parameter", function () {
            const receiver = container.get(FactoryIdentifier, {
                [paramFromContainer]: paramFromContainerValue,
            });
            chai_1.expect(receiver.paramFromFactory).to.equal(paramFromFactoryValue);
        });
    });
});
