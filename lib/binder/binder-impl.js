...
class BinderImpl {
    ...
    _ensureBoundOrBinding() {
        if (!this._binding) {
            ...
            if (injection_utils_1.isInjectable(this._identifier)) {
            ...
            else if (utils_3.isAutoBindFactory(this._identifier)) {
                ...
                throw new errors_1.BindingConfigurationError(`Binding for identifier "${utils_2.identifierToString(this._identifier)}" was never established.  Only @Factory functions or @Injectable classes may be auto-bound.`);
            }
        }
    }
    _finalizeBinding() {
