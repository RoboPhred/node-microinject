...
class BinderImpl {
    ...
    to(ctor) {
        ...
        this._binding = {
            type: "constructor",
            ctor,
            injections: injection_utils_1.getConstructorInjections(ctor)
        };
        return this;
    }
    ...
    inSingletonScope() {
        ...
        binding.inScope = symbols_1.SingletonSymbol;
    }
    /**
    ...
    asScope(scope) {
        ...
        binding.defineScope = symbols_1.SingletonSymbol;
    }
    ...
    _createDefaultBinding() {
        if (!this._binding) {
            ...
            else if (utils_2.isAutoBindFactory(this._identifier)) {
    ...
    _finalizeBinding() {
        ...
        if (this._binding && this._binding.type !== "value") {
            if (this._binding.defineScope === undefined)
                this._binding.defineScope = scope_utils_1.getAsScope(this._identifier);
            if (this._binding.inScope === undefined)
                this._binding.inScope = scope_utils_1.getInScope(this._identifier);
        }
    }
    _ensureCanBind() {
