...
class BinderImpl {
    ...
    _tryAutoBind() {
        if (!this._type) {
            ...
            if (utils_2.isInjectable(this._primaryIdentifier)) {
            ...
            else if (utils_4.isAutoBindFactory(this._primaryIdentifier)) {
                const factory = this._primaryIdentifier;
                ...
                throw new errors_1.BindingConfigurationError(`Binding for identifier "${utils_3.identifierToString(this._primaryIdentifier)}" was never established.  Only @Factory functions or @Injectable classes may be auto-bound.`);
            }
        }
    }
    _ensureCanBind() {
