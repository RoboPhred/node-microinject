...
exports.BasicDependencyGraphResolver = void 0;
...
function isNodeScoped(node) {
    return planner_1.isBindingDependencyNode(node) && binding_1.isScopeableBinding(node) && node.createInScope != null;
}
function isNodeScopeCreator(node) {
    return planner_1.isBindingDependencyNode(node) && binding_1.isScopeableBinding(node) && node.definesScope != null;
}
/**
...
class BasicDependencyGraphResolver {
    ...
    resolveInstance(node, opts = {}) {
        ...
        if (planner_1.isDynamicDependenencyNode(node)) {
            // Only dynamic nodes can cause cyclic dependencies.
            if (this.isInstantiating(node)) {
                throwCyclicDependency(node.identifier, this);
            }
            opts = Object.assign(Object.assign({}, opts), { parameters: Object.assign(Object.assign({}, ((_a = opts.parameters) !== null && _a !== void 0 ? _a : {})), node.parameters) });
        }
    ...
    _getScopedNodeInstance(node, opts) {
        ...
        const instance = this._createNodeInstance(node, opts, instance => this._scopedInstances.set(instanceId, instance));
        return instance;
    }
            ...
            switch (node.type) {
                ...
                case "param": {
                    ...
                    if (!utils_1.has(params, node.paramKey)) {
                        ...
                        throw new errors_1.ParameterNotSuppliedError(node.paramKey);
                    }
