...
class DependencyGraphPlanner {
     ...
     */
    getPlan(identifier, binding, noCache) {
        if (!binding) {
    ...
    _createDependencyNode(identifier, binding, scopeInstances) {
        if (binding.type === "value") {
            return Object.assign({}, binding, { identifier, instanceId: uuidv4() });
        }
        switch (binding.type) {
            case "factory": {
    ...
    _createFactoryNode(identifier, binding, scopeInstances) {
        const node = Object.assign({}, binding, { identifier, instanceId: uuidv4() });
    ...
    _createConstructorNode(identifier, binding, scopeInstances) {
        ...
        const node = Object.assign({}, binding, { identifier, instanceId: uuidv4(), ctor,
            ctorInjectionNodes,
            propInjectionNodes });
    ...
    _createValueInjection(injection, childScopeInstances) {
        ...
        if (all) {
            ...
            injectedArg = dependencyBindings.map(dependencyBinding => this._getDependencyNode(dependencyIdentifier, dependencyBinding, childScopeInstances));
        }
            ...
            injectedArg = this._getDependencyNode(dependencyIdentifier, dependencyBindings[0], childScopeInstances);
        }
        return injectedArg;
    }
