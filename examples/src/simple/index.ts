...
console.log(
  ...
  leftService.getLeftValue() + rightService.getRightValue()
);
...
console.log(
  "Are singletons ref equal?",
  singleton1 === singleton3 ? "yes" : "no"
);
...
for (let i = 0; i < scopeRoots.length; i++) {
  ...
  const sharedIDs = root
    .getShareConsumers()
    .map(
      x =>
        `{ consumerId: ${x.getConsumerId()}, scopeSharedId: ${x.getScopeSharedId()} }`
    );
  console.log(
    `Scope root ${i} has these ShareConsumers: [${sharedIDs.join(", ")}]`
  );
}
